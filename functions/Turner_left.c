...
/**
 * @brief Поворот изображения относительно оси Oy.
 * 
 * Функция выполняет зеркальное отражение изображения относительно вертикальной оси (оси Oy),
 * т.е. инвертирует координаты по горизонтали.
 * 
 ...
 * 
 * @note Эта функция меняет горизонтальное расположение пикселей, оставляя вертикальное положение неизменным.
 * 
 * @warning При использовании изображения с нечетной шириной может быть небольшая ошибка в выравнивании пикселей.
 */
